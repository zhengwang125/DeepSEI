...
def get_model():
    ...
    embed = layers.Embedding(13, 32, name='embedding', input_length=1)(input)
...
def train(X, Y):
    ...
    # checkpoint_dir = "./ckpt_2_0"
    checkpoint_path = "../KDD2022/entropy_pre/cp-loss{loss:.2f}.ckpt_2_0"
    checkpoint_dir = os.path.dirname(checkpoint_path)
...
def training():
    seq = np.arange(1, 13)
    skipgram, _ = keras.preprocessing.sequence.skipgrams(seq, 12, 2, 0)
    X = []
    Y = []
    for x, y in skipgram:
        X.append(x-1)
